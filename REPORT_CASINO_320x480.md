...
| Distribuzione | `11_distribuzione/casino/SNAI_BonusCasino_320x480_CORRETTO.mp4`: **1,57 MB** (originale 1,66 MB), H.264 High, 320×480, 15 fps, 14,00 s, senza audio, stessi tag colore |
...
**Modifica della v3:** l'importo SNAI 5.000€ è il nuovo asset aggiornato su `main` (commit `199cf3d`, `CASINO/…/text_5000.png`). Ho cambiato solo quella cifra.
- L'altezza resta **44 px**. A 44 px la cifra è larga **176 px** (v2: 169 px, v1: 152 px). L'arancio resta quello campionato dal master, (250,108,7).
- Il blocco SNAI resta centrato come prima e allineato a sinistra sull'importo, quindi logo e "FINO A" seguono il bordo sinistro della cifra: circa 3 px più a sinistra rispetto alla v2.
- Nell'assestamento f130–133 la scala di partenza è ricalcolata (284 px al f129 ÷ 175,7 = 1,617), così il passaggio dal conteggio originale resta identico.
- **Frame cambiati rispetto alla v2:** solo f130–133 e f135–210 (le righe 147–239, cioè il blocco SNAI). Tutti gli altri frame sono identici bit per bit. Riposo fermo (differenza 0 tra frame consecutivi); CTA e disclaimer identici all'originale (differenza 0).
- Tavola `07_zoom_nuovo_5000_f160.png` (3×; dall'alto: originale, v2, v3 MP4).
