# SNAI Bonus Sport 300×250: render corretto e QA

**Master di riferimento:** `00_master_approvato/SNAI_BonusSport_300x250_MASTER.mp4` (10,0 s, 150 frame, 15 fps).

| Consegna | File |
|---|---|
| Master lossless RGB | `09_rebuild_master/SNAI_BonusSport_300x250_CORRETTO_master_lossless_rgb.mkv` |
| Distribuzione | `11_distribuzione/SNAI_BonusSport_300x250_CORRETTO.mp4`: **580 KB** (originale 612 KB), H.264 High, 300×250, 15 fps, 10,00 s, senza audio, stessi tag colore |
| Confronto affiancato | `10_qa/300x250/confronto_originale_vs_corretto.mp4` |
| Tavole QA | `10_qa/300x250/01…06_*.png`, `TEST_pulse_SNAI_eliminato_f115-127.png`; misure in `qa_report.json` |

Stesse regole già approvate per gli altri formati:
- misure uniformi;
...
Le tre card compaiono **una alla volta**, nella stessa posizione (card originale 154×133, x 73–226,
y 23–155). Non c'è titolo. **In questo formato le card originali sono opache**: nessuna trasparenza
...
|---|---|---|---|---|
| William Hill (f60) | 156 × 135 | **22** | **10** | **37** |
| bet365 (f85) | 156 × 135 | **22** | **10** | **37** |
| SNAI (f112) | 156 × 135 | **22** | **10** | **37** |

- **Prima della correzione:** loghi 22/28/28, "FINO A" 10, importi 39/39/38.
- **Logo a 22 px:** è la misura del logo William Hill originale. SNAI e bet365 scendono da 28 px.
- **Importo a 37 px:** "2.000€" è largo 148 px su una card da 156, con circa 4 px di margine per lato,
  come l'originale (144 px su 154). A 35 px il margine salirebbe a circa 8 px per lato.
...
- **⚠ Espansioni (8 frame, 0,53 s): sfondo RICOSTRUITO.** La card ingrandita arrivava quasi al bordo
  alto (y 5) e oltre la parte alta della CTA. La fascia y 0–212 è interpolata con flusso ottico tra i
...
- **Fuori dalle card nessun pixel toccato** (esclusi gli 8 frame ricostruiti): differenza massima **0**
  sul master, 1,4 livelli medi sulla distribuzione.
...
- **Qualità:** PSNR tra distribuzione e master medio 39,6 dB, minimo 34,8 dB.

## Gate

Mi fermo qui finché non arriva un'approvazione esplicita del 300×250.
