...
cta = (slice(505, 548), slice(40, 260))
...
def measure(d, i):
    ...
    return out
rep['misure_riposo_distribuzione'] = {f: measure(d_dir, f) for f in (47, 60, 100)}
rep['misure_riposo_master'] = {f: measure(m_dir, f) for f in (47, 60, 100)}
...
stab = {}
for card, rng in [('snai', range(68, 124)), ('bet365', range(54, 64)), ('bet365', range(67, 124)),
                  ('wh', range(43, 51)), ('wh', range(52, 64)), ('wh', range(67, 124))]:
...
stab['originale_snai_f81-93_mean_abs_max'] = round(float(max(np.abs(L(o_dir, i + 1)[y0:y1, x0:x1] - L(o_dir, i)[y0:y1, x0:x1]).mean() for i in range(81, 93))), 3)
rep['stabilita'] = stab
