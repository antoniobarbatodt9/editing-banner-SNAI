...
Differenze rispetto agli altri formati (decisioni del cliente: A + P1 + importi 57 px):
 ...
 - il pulse SNAI (f90-99) copre sfondo in movimento mai visibile: si tolgono le contrazioni (f93-95, f99),
   le due espansioni (f90-92, f96-98) restano con l'ampiezza originale (opzione 1; la ricostruzione
   con flusso ottico e' stata testata e scartata per artefatti visibili a size reale);
...
# pulse SNAI (opzione 1 scelta dal cliente): nessuno sfondo ricostruito.
#  - fasi di ingrandimento (f90-92, f96-98): la card nuova segue l'ingombro della card originale
#    ingrandita (+1 px), perche' lo sfondo sotto non e' disponibile in nessun frame;
#  - fasi di restringimento (f93-95, f99): card a misura piena (copre gia' l'originale) -> rimosse.
for f, box in {90: (19, 97, 300, 340), 91: (-6, 74, 325, 363), 92: (-6, 74, 325, 363),
               96: (19, 97, 299, 340), 97: (20, 98, 299, 339), 98: (27, 103, 292, 333)}.items():
    put(f, 'snai', box, (box[3] - box[1] - 1) / (BOX[3] - BOX[1] - 1))
PULSE = {}
PULSE_FLOW = {}
...
QA_CTA_ROWS = (360, 414)
QA_REST = (49, 65, 85)
QA_STAB = [('wh', 44, 51), ('bet365', 61, 67), ('snai', 81, 89), ('snai', 93, 95), ('snai', 99, 115)]
